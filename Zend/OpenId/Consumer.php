...
class Zend_OpenId_Consumer
{
    ...
    private $_servers2_0 = array(
        //'http://openid.mail.ru/login',
        'https://www.google.com/accounts/o8/ud',
        'http://openid.yandex.ru/server/',
                ...
                if (isset($params['openid_claimed_id'])) {
                    ...
                    if (!Zend_OpenId::normalize($id)) {
                        ...
                        return false;
                                ...
                                !$this->equalIdentity($params["openid_identity"], $id)) ||
                               (!empty($params['openid_op_endpoint']) &&
                                $params['openid_op_endpoint'] != $discovered_server) ||
                               $discovered_version != $version) {
//Lms_Debug::debug(print_r(compact('params', 'id'),1));
    ...
    {
//Lms_Debug::debug(print_r(compact('id', 'server', 'version'),1));
//Lms_Debug::debug(print_r(debug_backtrace(false),1));
        $realId = $id;
        ...
        $expire = time() + 60 * 60;
//Lms_Debug::debug(print_r(compact('id', 'realId', 'server', 'version', 'expire'),1));
        ...
        $extensions=null, Zend_Controller_Response_Abstract $response = null)
    {
        ...
        if ($version <= 2.0) {
            if ($this->_session !== null) {
                ...
                if (in_array($server, $this->_servers2_0)) {
    ...
    * 
    */
    protected function equalIdentity($id1, $id2)
    {
        $openIdRegExp = '{openid\.yandex\.ru/([^/]*)}i';
        $yaRuRegExp = '{([^/\.]*)\.ya\.ru}i';
        $id1 = preg_replace($openIdRegExp, '$1', $id1);
        $id1 = preg_replace($yaRuRegExp, '$1', $id1);

        $id2 = preg_replace($openIdRegExp, '$1', $id2);
        $id2 = preg_replace($yaRuRegExp, '$1', $id2);
        return $id1 == $id2;
    }
